...
import com.example.my12306_ymc.bean.Account;
import com.example.my12306_ymc.my.MyAccountActivity;
import com.example.my12306_ymc.my.MyAccountActivity.AccountTask;
...
public class MyAccountActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter = new SimpleAdapter(MyAccountActivity.this, data, 
		...
		row3.put("key1", "证件号码");
		row3.put("key2", "410603199590440848");
		row3.put("key3", null);
		data.add(row3);
	...
	class AccountTask extends AsyncTask<String, String, Object> {
		...
		protected void onPostExecute(Object result) {
			...
			if (result instanceof Account) {
				...
				Map<String, Object> row0 = new HashMap<String, Object>();
				row0.put("key1", "用户名");
				row0.put("key2", ((Account) result).getId());
				row0.put("key3", null);
				data.add(row0);
				...
				row1.put("key1", "姓名");
				row1.put("key2", ((Account) result).getName());
				row1.put("key3", null);
				data.add(row1);
				...
				row2.put("key1", "证件类型");
				row2.put("key2",((Account) result).getIdType() );
				row2.put("key3", null);
				data.add(row2);
				...
				Map<String, Object> row3 = new HashMap<String, Object>();
				row3.put("key1", "证件号码");
				row3.put("key2", ((Account) result).getId());
				row3.put("key3", null);
				data.add(row3);
				...
				Map<String, Object> row4 = new HashMap<String, Object>();
				row4.put("key1", "乘客类型");
				row4.put("key2", ((Account) result).getType());
				row4.put("key3", R.drawable.forward_25);
				data.add(row4);
				...
				row5.put("key1", "电话");
				row5.put("key2",((Account) result).getTel());
				row5.put("key3", R.drawable.forward_25);
				data.add(row5);
		...
		public void handleMessage(Message msg)
		{
			...
			case 1:
				...
				if ("1".equals(result)) {
					...
					Toast.makeText(MyAccountActivity.this,"添加联系人成功",
							Toast.LENGTH_SHORT).show();
					finish();
				} else if ("-1".equals(result)) {
					Toast.makeText(MyAccountActivity.this, "添加联系人失败，请重试",
							Toast.LENGTH_SHORT).show();
				}
				break;
			case 2:
